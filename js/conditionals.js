...
// alert("Your total price after the discount is " + calculateTotal(luckyNumber,TB))
    ...
    function evenOrOdd(x) {
        ...
        if (userNumber % 2 === 0) {
            ...
            return alert("You have not entered a value")
        }
    }
    ...
    function posOrNeg(x) {
        ...
        } else "Your number is 0, or not a value"
    }

    function numAnd100(x) {
        var usernum = x
        return alert("Your number + 100 is " + (parseFloat(usernum) + 100))
    }

    function inputNotGood(x) {
        var nv = x
        alert(nv + " is not a good value, try again")
    }


    var okOrCancel = confirm("Would you like to enter a number?")
    if (okOrCancel == true) {
